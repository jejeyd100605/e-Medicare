...
function teamChipHTML(assignedTo){
    if(!assignedTo) return '<span class="team-none">— Unassigned —</span>';
    // "Accident, Alai (Vehicle: X, Driver: Y)"  ->  title + members
    const m = String(assignedTo).match(/^(.*?)\s*\((.*)\)\s*$/);
    const title = m ? m[1] : assignedTo;
    const parts = m ? m[2].split(/,\s*(?=(?:Vehicle|Driver|Responder):)/) : [];
    return `
        <div class="team-chip">
            <div class="team-title">👥 ${title}</div>
            ${parts.length ? `<div class="team-members">${parts.join(' · ')}</div>` : ''}
        </div>`;
}
...
function renderUsers(){
  ...
  // unseen muna, tapos verified sa taas, hindi verified sa baba
  const sortedUsers = sortUnseenFirst(usersCache, 'users')
    .map((u, i) => ({ u, i }))
    .sort((a, b) => (b.u.id_verified ? 1 : 0) - (a.u.id_verified ? 1 : 0) || a.i - b.i)
    .map(x => x.u);


  tbody.innerHTML = sortedUsers.map(u => {
    const active = u.active !== false;
    const roleLabel = u.role === 'responder' && u.position ? u.position : u.role;
    return `
    <tr>
      <td>
        ...
        <div class="role-cell">
          <span>${roleLabel}</span>
          ${u.id_verified ? '<span class="id-badge">ID Verified</span>' : ''}
        </div>
      </td>
      <td>${active ? '<span class="status-pill status-Available">Active</span>' : '<span class="status-pill status-Unavailable">Inactive</span>'}</td>
      <td>
        <div class="user-actions">
          <button class="primary-btn act-btn" style="background:${active ? '#3a2a10' : '#123a1c'};color:${active ? '#ffcc66' : '#8aff9c'};" onclick="toggleUserActive('${u.id}')">${active ? 'Deactivate' : 'Activate'}</button>
          <button class="primary-btn act-btn" style="background:#333;color:#eee;" onclick="openSecurityModal('${u.id}')">Forced Reset Password</button>
          ${u.id_image_url ? `<button class="primary-btn act-btn" style="background:#123a4a;color:#66d9ff;" onclick="openIdVerificationModal('${u.id}')">🪪 Review ID</button>` : ''}
        </div>
      </td>
    </tr>`;
  }).join('');
